#!/usr/bin/env bash
# Requires sudo.
# Adds MDM enrollment check, thermal/fan/power data, and deeper system logs.

if [[ "$EUID" -ne 0 ]]; then
  echo "This script must be run with sudo: sudo ./check_mac_sudo.sh"
  exit 1
fi

TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
OUTPUT_FILE="mac_report_full_${TIMESTAMP}.txt"
...
# MDM enrollment — the critical second-hand check
if echo "$MDM_STATUS" | grep -q "Enrolled via DEP: No" && echo "$MDM_STATUS" | grep -q "MDM enrollment: No"; then
  add_line "PASS" "MDM enrollment: Not enrolled  (not locked to a company)"
elif echo "$MDM_STATUS" | grep -q "MDM enrollment: Yes"; then
  add_line "FAIL" "MDM enrollment: ENROLLED  (locked to an organisation — machine may be unusable)"
elif echo "$MDM_STATUS" | grep -q "Enrolled via DEP: Yes"; then
  add_line "WARN" "MDM enrollment: DEP enrolled  (was managed — confirm MDM was removed)"
else
  add_line "????" "MDM enrollment: $MDM_STATUS"
fi
...
print_details() {
  ...
  echo "$POWER_DATA" | grep -E "Cycle Count|Maximum Capacity|Condition|Charging|Full Charge Capacity|Design Capacity|State of Charge|Manufacturer|Device Name"
  echo ""
  pmset -g batt
  ...
  section "THERMAL / FANS / POWER (5-second sample)"
  echo "  Look for: throttling flags, fan RPM, CPU/GPU die temperature, power draw"
  echo ""
  powermetrics --samplers smc,cpu_power,gpu_power -n 1 -i 5000 2>/dev/null \
    || echo "  powermetrics not available"

  section "UPTIME"
  uptime

  section "MDM ENROLLMENT DETAIL"
  echo "$MDM_STATUS"
  ...
  ls -lt /Library/Logs/DiagnosticReports/ ~/Library/Logs/DiagnosticReports/ 2>/dev/null | head -12 \
...
print_benchmarks() {
  ...
  echo "  Healthy Apple Silicon: 2000+ sign/s"
